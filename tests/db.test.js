...
test('saveGameResult updates endless stats and trims history', async () => {
  const { loadStats, saveGameResult } = await setupDbModule();

  let best = false;
  for (let index = 0; index < 12; index += 1) {
    best = await saveGameResult(index + 1, index % 2 === 0, 'endless');
  }

  const stats = await loadStats();

  assert.equal(best, true);
  assert.equal(stats.highscoreEndless, 12);
...
test('saveGameResult tracks endless best independently', async () => {
  const { loadStats, saveGameResult } = await setupDbModule();
